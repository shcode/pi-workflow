...
export default function pmaiQna(pi: ExtensionAPI) {
  pi.on("tool_call", async (event, ctx) => {
    // Only intercept questionnaire tool in json mode with pmai contract
    if (ctx.mode !== "json" || !isPmaiMode()) return;
    ...
    if (!answersEvent) {
      // Stdin closed or parse error (e.g. timeout killed the session)
      return {
        block: true,
    ...
    // Format answers as tool result text — LLM receives this as the questionnaire response
    ...
    // Block the normal questionnaire TUI execution and return our answer text instead
    return {
      block: true,
      reason: answerText,
    };
